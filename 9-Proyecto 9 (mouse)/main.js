...
function touchmove(e){
    cursorPos = {x: e.targetTouches[0].elOffset,y:e.targetTouches[0].offsetY};
    seguir();
...
const seguirCursor=(el,xrelacion,yrelacion)=>{
    ...
    const distanceLeft = Math.round(((cursorPos.x - centerY)*100)/window.innerWidth);
    const distanceTop = Math.round(((cursorPos.y - centerX)*100)/window.innerHeight);
